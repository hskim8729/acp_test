<template>
  ...
  <div class="m_file_attach">
    <file-handler table="TBM" :fileKey="existingFile" ref="fileHandlerRef" />
  </div>
...
const searchList = async () => {
  ...
  if (tbmDetailMap.value.atchFileKey) {
    existingFile.value = tbmDetailMap.value.atchFileKey;
  }
};
...
watch(
  () => fileHandlerRef.value?.newUploadFiles, // Getter function to access newUploadFiles
  (newValue, oldValue) => {
    if (newValue) {
      alert("fileHandlerRef");
    }
  },
  { deep: true }
);
