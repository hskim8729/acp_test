    ...
    <div class="popup_content">
      <div class="left">
        <div class="subtitle_area">
          ...
          <div class="right_btn">
            <!-- 검색기능 -->
            <input type="text" class="search_input_h32" placeholder="분류 및 작업 검색" v-model="searchQuery"
              @keyup.enter.prevent="togglePopup" />
            <button class="search_btn" @click="togglePopup"></button>
            <popSearchStandard :defaultColDef="defaultColDef" :localeText="localeText"
              :vendrCd="$loginStore.$state.siteInfo.vendrCd" :siteCd="$loginStore.$state.siteInfo.siteCd"
              :siteHrmrskFctrMstSeq="emitDtl.siteHrmrskFctrMstSeq" :isOpenPopup="isOpenPopup" :searchQuery="searchQuery"
              @togglePopup="togglePopup" v-if="isOpenPopup" />
          </div>
        </div>
        ...
        <div class="content_box">
          <div class="std_sort_step1">
            ...
            <div class="grid_sort">
              <table>
                <tbody id="lgTbody">
                  <tr tabIndex=0 v-for="(item, index) in lgCateList.value" :key="index">

                    <!-- 대분류 체크 된 항목-->
                    <td v-if="index === selectedLgCatRowIndex" :class="{ checked: index === selectedLgCatRowIndex }"
                      @click="selectFetchLgCatList(item, index, allParam)" style="
                        display: flex;
                    ...
                    <!-- 대분류 체크 안된 항목-->
                    <td v-if="index != selectedLgCatRowIndex" @click="selectFetchLgCatList(item, index, allParam)" style="
                        display: flex;
                        justify-content: space-between;
                        align-items: center;
                      ">
            ...
            <div class="grid_sort">
              <table>
                <tbody id="mdTbody">
                  ...
                  <tr tabIndex=0 v-for="(item, index) in mdCateList.value" :key="index">
                    <td :class="{ checked: index === selectedMdCatRowIndex }" @click="selectFetchMdCatList(item, index)"
                      style="
                        display: flex;
                        justify-content: space-between;
                        align-items: center;
          ...
          <div class="std_sort_step3">
            ...
            <div class="grid_sort">
              <table>
                <tbody id="wrkTbody">
                  <tr tabIndex=0 v-for="(item, index) in wrkCateList.value" :key="index">
                    <td :class="{ checked: index === selectedWrkCatRowIndex }" @click="selectFetchWrkCatList(item, index)"
                      style="
                        display: flex;
                        justify-content: space-between;
                        align-items: center;
      ...
      <div class="right">
        ...
        <div class="content_box risk">
          ...
          <div class="grid_multichk">
            <table>
              <tr class="multi_chk_row" v-for="(item, index) in rskFctrCateList.value" :key="index">
                <td @click="selectFetchRskFctrCatList(item, index)">
                  <input type="checkbox" class="multi_chk_left" :value="item.rskFctr"
                    :data-id="item.rskFctrSeq + '/@' + item.rskFctr" />{{ item.rskFctr }}
                  <!-- 왼쪽 항목 -->
                </td>
                <td>
                  <table v-for="(item2, index2) in rskMsrCateList.value" :key="index2">
                    <tr v-if="item.rskFctrSeq === item2.rskFctrSeq">
                      <td @click="selectFetchRskMsrCatList(item2, item)">
                        <input type="checkbox" class="multi_chk_right" :value="item2.rskMsr"
                          :data-id="item.rskFctrSeq + '/@' + item2.rskMsr" />{{ item2.rskMsr }}
                        <!-- 오른쪽 항목 -->
                      </td>
                    </tr>
                  </table>
...
async function selectFetchLgCatList(lgItem, index, allParam) {
  ...
  selectedLgCatRowIndex.value = index === undefined || index === null ? 0 : index;
  document.getElementById("lgTbody").children[selectedLgCatRowIndex.value].focus();
...
async function selectFetchMdCatList(mdItem, index, allParam, cnt) {
  ...
  chkReset();
  // 중분류 row index 값
  selectedMdCatRowIndex.value = index === undefined || index === null ? 0 : index;
  ...
  // 중분류 포커스 주기
  document.getElementById("mdTbody").children[selectedMdCatRowIndex.value].focus();
...
async function selectFetchWrkCatList(wrkItem, index) {
  ...
  document.getElementById("wrkTbody").children[selectedWrkCatRowIndex.value].focus();
...
onMounted(async () => {
  watch(() => lgCateList.value, (newVal, oldVal) => {
