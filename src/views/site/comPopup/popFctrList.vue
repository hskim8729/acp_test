  ...
  <div class="popup add_fctr">
    ...
    <div class="popup_content">
      <div class="grid_sort3">
        <table>
          <tr v-for="(item, index) in fctrList.value" :key="index">
            <td>
              <input
                type="checkbox"
                :name="rskFctr"
                :id="item.rskFctrSeq"
                :value="item.rskFctrSeq"
                @change="selectedItem($event, index)"
                :checked="checked(item.rskFctrMarkYn)" />
              {{ item.rskFctr }}
            </td>
            <!-- <td>{{ item.rskFctrMarkYn }}</td> -->
          </tr>
        </table>
      </div>
    </div>
...
onBeforeMount(async () => {
  ...
  // 작업명값이 있을때
  if (!cUtil.nullCheck(props.codeParams.wrkNmCd)) {
    // 위험요인 조회
    fctrList.value = await siteStore.fetchRskFctrList(params);
  }
});
...
function selectedItem(event, index) {
  const dataList = fctrList.value.filter((item, idx) => idx === index);
  ...
  const saveList = dataList.map((item) => ({
    ...
    userId: userId,
    rskFctrMarkYn: item.rskFctrMarkYn == "N" ? "Y" : "N",
  }));
...
async function addMarkYn() {
  ...
  // 위험요인 표시여부 변경
  const res = await siteStore.fetchSaveSiteRiskFctrList(saveParams);
...
function popupClose() {
  ...
  document.getElementsByClassName("popup add_fctr")[0].classList.remove("show");
}
</script>
